    ...
    <div>
      ...
      <div className='trending'>
                {content && content.map((el) => (
                    <SingleContent key={el.id} id={el.id} poster={el.poster_path} title={el.title || el.name} date={el.first_air_date || el.release_date} media_type='movie' vote_average={el.vote_average} />
                ))}
            </div>
            {numOfPages>1 && (
